...
// 给出节点名，返回节点值
- (NSString *)stringValueByName:(NSString*)name {
    NSError *error = nil;
    GDataXMLElement *element = [self nodesForXPath:name error:&error][0];
    if (!error) {
...
// 给出节点名，返回节点名对应的所有的节点值，并把它们放在数组中返回
- (NSArray *)stringValuesByName:(NSString*)name {
    NSMutableArray *mArr = [NSMutableArray array];
    NSArray *array = [self elementsForName:name];
    for (GDataXMLElement *element in array) {
        [mArr addObject:element.stringValue];
    }
    return mArr;
}

// 根据属性名获得属性值
- (NSString *)attributeStringValueByName:(NSString *)name {
    // 先根据属性名，取出属性节点
...
- (NSDictionary *)subDictWithArray:(NSArray*)names {
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    for (NSString *name in names) { // name是节点名
        [dict setValue:[self stringValueByName:name] forKey:name];
    }
    
    return dict;
}

// 根据dict字典创建xml节点，在节点element下插入新节点（这个新节点是根据dict的内容创建的）
// 根据dict字典创建xml节点, 加到节点element下
/**
 *
 @"name" : @"呐喊", --> <name>呐喊</name>
 
 @"author" : @"振哥", --> <author>振哥</author>
 
 @"otherInfo" : @{@"wife" : @"文清", @"husband" : @"天栋"} -->
 <therInfo>
    <wife>文清</wife>
    <husband>天栋</husband>
 </otherInfo>
...
- (void)xmlElementFromDict:(NSDictionary*)dict element:(GDataXMLElement *)element {
    ...
    for (NSString *key in keys) { // 遍历字典中所有的key
        id obj = [dict objectForKey:key]; // 找到key对应的值
        if ([obj isKindOfClass:[NSDictionary class]]) { // 如果值是字典，递归加节点
            GDataXMLElement *childElement = [GDataXMLElement elementWithName:key];
            [self xmlElementFromDict:obj element:childElement]; // 递归, 把obj这个字典生成新的节点作为childElement的子节点
            [element addChild:childElement];
        } else {
            GDataXMLElement *childElement = [GDataXMLElement elementWithName:key stringValue:obj]; // 根据key-value生成新节点，key作为节点名，obj作为节点的值
            [element addChild:childElement]; // 把新节点加进当前节点中
        }
    }
}
