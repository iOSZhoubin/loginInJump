...
- (void)viewDidLoad {
    ...
    self.navigationItem.title = @"网络连接";
...
//    self.navigationController.navigationBar.translucent = NO;
    ...
    _subView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth  , ScreenHeight-L2C_TabbarHeight)];
...
-(void)getMobileconfigFile:(id)sender{
    ...
    if (mobileconfigField.text.length < 1 || serField.text.length < 1 || userField.text.length < 1 ||passField.text.length < 1) {
        ...
        if ([VPNCheckoutTextField checkmobileconfigField:mobileconfigField.text]) {
            ...
            NSString *string = [NSString stringWithFormat:@"https://%@/createXml.php?p1=%@&p2=%@&p3=%@&p4=%@&p5=%@",SafeString(_dataDict[@"server"]),SafeString(userField.text),SafeString(passField.text),SafeString(serField.text),SafeString(mobileconfigField.text),SafeString(key.text)];
